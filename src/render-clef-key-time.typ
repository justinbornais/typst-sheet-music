...
#import "glyph-metadata.typ": place-glyph, advance-width, glyph-placement
...
#let clef-placement(x, y-top, clef-name, sp: 1.0, scale: 1.0, music-font-config: none) = {
  let smufl = clef-smufl-name.at(clef-name, default: "gClef")
  ...
  glyph-placement(x, origin-y, glyph, smufl, sp * scale, config: music-font-config)
}

#let key-signature-placements(x, y-top, key-name, clef-name, sp: 1.0, music-font-config: none) = {
  let count = key-sig-accidental-count.at(key-name, default: 0)
  if count == 0 { return () }
  ...
  for i in range(n) {
    ...
    result.push(glyph-placement(acc-x, acc-y, acc-glyph, acc-smufl, sp, config: music-font-config))
  }
  result
}

#let time-signature-placements(x, y-top, upper, lower, symbol: none, sp: 1.0, music-font-config: none) = {
  if upper == none and symbol == none { return () }

  if symbol == "common" {
    return (glyph-placement(x, y-top - 2.0 * sp, smufl-time-common, "timeSigCommon", sp, config: music-font-config),)
  }
  if symbol == "cut" {
    return (glyph-placement(x, y-top - 2.0 * sp, smufl-time-cut, "timeSigCutCommon", sp, config: music-font-config),)
  }
  ...
  for ch in upper-str {
    if ch >= "0" and ch <= "9" {
      ...
      result.push(glyph-placement(x + dx, upper-y, glyph-char, glyph-name, sp, config: music-font-config))
  ...
  for ch in lower-str {
    if ch >= "0" and ch <= "9" {
      let glyph-name = "timeSig" + ch
      let glyph-char = smufl-time-digits.at(int(ch))
      result.push(glyph-placement(x + dx, lower-y, glyph-char, glyph-name, sp, config: music-font-config))
      dx += advance-width(glyph-name, config: music-font-config) * sp
    }
  }
  result
...
#let draw-clef(x, y-top, clef-name, sp: 1.0, scale: 1.0, music-font-config: none, emit: none) = {
  let smufl = clef-smufl-name.at(clef-name, default: "gClef")
  let origin-offset = clef-origin-offset.at(clef-name, default: 3.0)
  let glyph = clef-config.at(clef-name).glyph
  let origin-y = y-top - origin-offset * sp
  place-glyph(x, origin-y, glyph, smufl, sp * scale, config: music-font-config, emit: emit)
}

/// Draw a key signature at the given position (no return value).
#let draw-key-signature(x, y-top, key-name, clef-name, sp: 1.0, music-font-config: none, emit: none) = {
  let count = key-sig-accidental-count.at(key-name, default: 0)
  if count == 0 { return }
  ...
  for i in range(n) {
    ...
    let acc-y = y-top - staff-pos * sp / 2.0
    let acc-x = x + i * acc-spacing
    place-glyph(acc-x, acc-y, acc-glyph, acc-smufl, sp, config: music-font-config, emit: emit)
  }
}

/// Draw a time signature at the given position (no return value).
#let draw-time-signature(x, y-top, upper, lower, symbol: none, sp: 1.0, music-font-config: none, emit: none) = {
  // If no explicit time signature was provided, do nothing.
  if upper == none and symbol == none { return }

  if symbol == "common" {
    place-glyph(x, y-top - 2.0 * sp, smufl-time-common, "timeSigCommon", sp, config: music-font-config, emit: emit)
    return
  }

  if symbol == "cut" {
    place-glyph(x, y-top - 2.0 * sp, smufl-time-cut, "timeSigCutCommon", sp, config: music-font-config, emit: emit)
    return
  }
  ...
  for ch in upper-str {
    if ch >= "0" and ch <= "9" {
      let glyph-name = "timeSig" + ch
      let glyph-char = smufl-time-digits.at(int(ch))
      place-glyph(x + dx, upper-y, glyph-char, glyph-name, sp, config: music-font-config, emit: emit)
      dx += advance-width(glyph-name, config: music-font-config) * sp
    }
  }
  ...
  for ch in lower-str {
    if ch >= "0" and ch <= "9" {
      let glyph-name = "timeSig" + ch
      let glyph-char = smufl-time-digits.at(int(ch))
      place-glyph(x + dx, lower-y, glyph-char, glyph-name, sp, config: music-font-config, emit: emit)
      dx += advance-width(glyph-name, config: music-font-config) * sp
    }
  }
}
