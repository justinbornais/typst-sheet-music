...
#import "glyph-metadata.typ": font-family, place-glyph, advance-width, bbox, content-placement, glyph-placement

#let articulation-placements(x, note-y, articulations, stem-dir, y-top, sp: 1.0, music-font-config: none) = {
  ...
  if art-above {
    ...
    for art in sorted {
      ...
      if glyph != none {
        let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, glyph)
        result.push(content-placement(x, cur-y, body, anchor: "south"))
        cur-y += art-spacing
      }
    }
  } else {
    ...
    for art in sorted {
      ...
      if glyph != none {
        let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, glyph)
        result.push(content-placement(x, cur-y, body, anchor: "north"))
        cur-y -= art-spacing
      }
    }
  }
  ...
  if fermata.len() > 0 {
    ...
    let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, smufl-articulations.fermata-above)
    result.push(content-placement(x, fermata-y, body, anchor: "south"))
  }

  result
}

#let dynamic-placement(x, y-bottom, dynamic, sp: 1.0, extra-offset: 0.0, music-font-config: none) = {
  if dynamic == none or dynamic == "" { return none }
  ...
  if all-smufl {
    ...
    let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, glyph-str)
    return content-placement(x, dyn-y, body, anchor: "north")
  }

  let body = text(size: 8pt, style: "italic", weight: "bold", dynamic)
  content-placement(x, dyn-y, body, anchor: "north")
}

#let trill-symbol-placement(x, y, sp: 1.0, music-font-config: none) = {
  glyph-placement(x, y, smufl-ornaments.trill, "ornamentTrill", sp, config: music-font-config)
}

#let trill-wiggle-placements(x-start, x-end, y, sp: 1.0, music-font-config: none) = {
  if x-end <= x-start { return () }

  let seg-w = advance-width("wiggleTrill", config: music-font-config) * sp
  if seg-w <= 0 { return () }
  ...
  let cur-x = x-start
  while cur-x < x-end {
    result.push(glyph-placement(cur-x, y, smufl-ornaments.wiggle-trill, "wiggleTrill", sp, config: music-font-config))
    cur-x += step
  }
  result
}

#let staff-marker-placement(x, y, kind, sp: 1.0, music-font-config: none, anchor-mode: "south") = {
  ...
  if glyph == none or smufl == none { return none }
  let size = default-music-font-size-factor * scale * sp * 1mm
  let body = text(font: font-family(config: music-font-config), size: size, glyph)
  let bb = bbox(smufl, config: music-font-config)
  if anchor-mode == "south-west" {
    return content-placement(x, y, body, anchor: "south-west")
  } else if anchor-mode == "center-west" and bb != none {
    let height = (bb.ne.y - bb.sw.y) * scale * sp
    return content-placement(x, y - 0.5 * height, body, anchor: "south-west")
  }
  content-placement(x, y, body, anchor: "south")
}
...
#let draw-articulations(x, note-y, articulations, stem-dir, y-top, sp: 1.0, music-font-config: none, emit: none) = {
  ...
  if art-above {
    ...
    for art in sorted {
      ...
      if glyph != none {
        let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, glyph)
        if emit != none {
          emit(x, cur-y, body, anchor: "south")
        } else {
          content((x, cur-y), anchor: "south", body)
        }
        cur-y += art-spacing
      }
    }
    ...
    for art in sorted {
      ...
      if glyph != none {
        let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, glyph)
        if emit != none {
          emit(x, cur-y, body, anchor: "north")
        } else {
          content((x, cur-y), anchor: "north", body)
        }
        cur-y -= art-spacing
      }
    }
  ...
  if fermata.len() > 0 {
    ...
    let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, smufl-articulations.fermata-above)
    if emit != none {
      emit(x, fermata-y, body, anchor: "south")
    } else {
      content((x, fermata-y), anchor: "south", body)
    }
  }
}
...
#let draw-dynamic(x, y-bottom, dynamic, sp: 1.0, extra-offset: 0.0, music-font-config: none, emit: none) = {
  import "@preview/cetz:0.4.2"
  import cetz.draw: *
  ...
  if all-smufl {
    ...
    let body = text(font: font-family(config: music-font-config), size: default-music-font-size-factor * sp * 1mm, glyph-str)
    if emit != none {
      emit(x, dyn-y, body, anchor: "north")
    } else {
      content((x, dyn-y), anchor: "north", body)
    }
  } else {
...
#let draw-trill-symbol(x, y, sp: 1.0, music-font-config: none, emit: none) = {
  place-glyph(x, y, smufl-ornaments.trill, "ornamentTrill", sp, config: music-font-config, emit: emit)
}

#let draw-trill-wiggle(x-start, x-end, y, sp: 1.0, music-font-config: none, emit: none) = {
  if x-end <= x-start { return }
  ...
  let cur-x = x-start
  while cur-x < x-end {
    place-glyph(cur-x, y, smufl-ornaments.wiggle-trill, "wiggleTrill", sp, config: music-font-config, emit: emit)
    cur-x += step
  }
}

#let draw-staff-marker(x, y, kind, sp: 1.0, music-font-config: none, anchor-mode: "south", emit: none) = {
  import "@preview/cetz:0.4.2"
  import cetz.draw: *
  let glyph = if kind == "breath-mark" { smufl-other.breath-mark }
    else if kind == "caesura" { smufl-other.caesura }
  ...
  if anchor-mode == "south-west" {
    if emit != none {
      emit(x, y, body, anchor: "south-west")
    } else {
      content((x, y), anchor: "south-west", body)
    }
  } else if anchor-mode == "center-west" and bb != none {
    let height = (bb.ne.y - bb.sw.y) * scale * sp
    if emit != none {
      emit(x, y - 0.5 * height, body, anchor: "south-west")
    } else {
      content((x, y - 0.5 * height), anchor: "south-west", body)
    }
  } else {
    if emit != none {
      emit(x, y, body, anchor: "south")
    } else {
      content((x, y), anchor: "south", body)
    }
  }
}
