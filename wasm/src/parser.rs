...
fn is_whitespace_char(ch: u8) -> bool { ch == b' ' || ch == b'\t' || ch == b'\r' }
fn is_word_char(ch: u8) -> bool { is_lower(ch) || is_digit(ch) || ch == b'-' }
...
impl<'a> Parser<'a> {
    fn new(input: &'a str, base_octave: i32) -> Self {
        Parser {
            ...
            hairpin_start_idx: None, hairpin_kind: None, hairpin_open_order: None,
            trill_start_idx: None, trill_open_order: None,
            grace_start_idx: None, grace_slash: false, grace_open_order: None,
            ending_start_idx: None, ending_label: None, ending_open_order: None,
        }
    }

    fn len(&self) -> usize { self.input.len() }
    fn peek(&self, p: usize) -> Option<u8> {
        if p < self.len() { Some(self.input[p]) } else { None }
    }
    fn _peek_char(&self, p: usize) -> Option<char> {
        self.peek(p).map(|b| b as char)
    }
    fn ch(&self) -> u8 { self.input[self.pos] }

    fn next_nonspace_char(&self, mut p: usize) -> Option<u8> {
        while p < self.len() && is_whitespace_char(self.input[p]) { p += 1; }
        self.peek(p)
    }
    ...
    fn parse_accidental(&self, mut p: usize) -> (Option<String>, usize) {
        match self.peek(p) {
            Some(b'#') => {
                p += 1;
                if self.peek(p) == Some(b'#') { p += 1; (Some("double-sharp".into()), p) }
                else { (Some("sharp".into()), p) }
            }
            Some(b'&') => {
                p += 1;
                if self.peek(p) == Some(b'&') { p += 1; (Some("double-flat".into()), p) }
                else { (Some("flat".into()), p) }
            }
    ...
    fn parse_octave_markers(&self, mut p: usize, mut octave: i32) -> (i32, usize) {
        while self.peek(p) == Some(b'\'') { octave += 1; p += 1; }
        while self.peek(p) == Some(b',') { octave -= 1; p += 1; }
        (octave, p)
    }
    ...
    fn parse_duration_dots(&self, mut p: usize, sticky: i32) -> (i32, i32, usize) {
        ...
        while let Some(ch) = self.peek(p) {
            if is_digit(ch) { dur_str.push(ch as char); p += 1; } else { break; }
        }
        let duration = if dur_str.is_empty() { sticky } else { dur_str.parse::<i32>().unwrap_or(sticky) };
        let mut dots = 0;
        while self.peek(p) == Some(b'.') { dots += 1; p += 1; }
        (duration, dots, p)
    }
    ...
    fn parse_fingering(&self, p: usize) -> (Option<Fingering>, String, usize) {
        ...
        let fingering = if parts.len() == 1 {
            ...
            if vals.is_empty() { None } else { Some(Fingering::Multiple(vals)) }
        } else { None };
    ...
    fn parse_lyric(&self, p: usize) -> (Option<LyricEntry>, usize) {
        if self.peek(p) != Some(b'l') { return (None, p); }
        if p + 1 < self.len() && self.input[p + 1] == b'[' {
            ...
            (Some(LyricEntry { text: Some(text), carry: false, continuation }), next_pos)
        } else {
            (Some(LyricEntry { text: None, carry: true, continuation: "none".to_string() }), p + 1)
        }
    }

    fn parse_tagged_text(&self, p: usize, tag: &str) -> (Option<String>, usize) {
        let tag_bytes = tag.as_bytes();
        if p + tag_bytes.len() >= self.len() { return (None, p); }
        if &self.input[p..p + tag_bytes.len()] != tag_bytes { return (None, p); }
        if self.input[p + tag_bytes.len()] != b'[' { return (None, p); }
    ...
    fn parse_staff_marker(&self, p: usize) -> (Option<String>, usize) {
        if self.peek(p) == Some(b'c') && self.peek(p+1) == Some(b'o')
            && self.peek(p+2) == Some(b'd') && self.peek(p+3) == Some(b'a') {
    ...
    fn parse_note_attachments(&self, mut p: usize) -> (NoteAttachments, usize) {
        ...
        if self.peek(p) == Some(b'~') { att.tie = true; p += 1; }

        // Articulations
        loop {
            match self.peek(p) {
                Some(b'>') => { att.articulations.push("accent".into()); p += 1; }
                Some(b'*') => { att.articulations.push("staccato".into()); p += 1; }
                Some(b'-') => { att.articulations.push("tenuto".into()); p += 1; }
                Some(b'_') => { att.articulations.push("fermata".into()); p += 1; }
                _ => break,
            }
        }
        ...
        if self.peek(p) == Some(b'v') && p + 1 < self.len() && self.input[p + 1] == b'[' {
            ...
            if !value.is_empty() { att.dynamic = Some(value); }
            p = next_pos;
        }

        // Tie after articulations/dynamic
        if !att.tie && self.peek(p) == Some(b'~') { att.tie = true; p += 1; }
        ...
        loop {
            ...
            if let Some(m) = marker { att.staff_markers.push(m); p = next_p; }
            else { break; }
        }

        // Slur start/end
        if self.peek(p) == Some(b'(') { att.slur_start = true; p += 1; }
        if self.peek(p) == Some(b')') { att.slur_end = true; p += 1; }

        // Beam start/end
        if self.peek(p) == Some(b'[') {
            let nxt = if p + 1 < self.len() { Some(self.input[p + 1]) } else { None };
            let is_chord_bracket = nxt.map_or(false, |c| c >= b'A' && c <= b'G');
            if !is_chord_bracket { att.beam_start = true; p += 1; }
        }
        if self.peek(p) == Some(b']') { att.beam_end = true; p += 1; }

        // Chord symbol, staff text, expression text, fingering, lyrics
        loop {
            if self.peek(p) == Some(b'l') {
                let (entry, next_p) = self.parse_lyric(p);
                if let Some(e) = entry { att.lyrics.push(e); }
                p = next_p;
                ...
                if !value.is_empty() { att.chord_symbol = Some(value); }
                p = next_p;
            } else {
                break;
            }
    ...
    fn parse_note_event_data(&self, p: usize) -> (NoteEventData, usize) {
        ...
        (NoteEventData { accidental, octave, duration, dots, att }, p4)
    }

    fn parse_time_token(&self, token: &str) -> Option<Event> {
        match token {
            "common" | "C" => Some(Event::TimeSig(TimeSig {
                upper: 4, lower: 4, symbol: Some("common".into()),
                ending: None, ending_start: false, ending_end: false,
            })),
            "cut" | "C|" => Some(Event::TimeSig(TimeSig {
                upper: 2, lower: 2, symbol: Some("cut".into()),
                ending: None, ending_start: false, ending_end: false,
            })),
            _ if token.contains('/') => {
    ...
    fn parse(&mut self) {
        while self.pos < self.len() {
            ...
            if is_whitespace_char(ch) {
                let start = self.pos;
                while self.pos < self.len() && is_whitespace_char(self.input[self.pos]) { self.pos += 1; }
                ...
                if run_len > 1 && prev_event.is_some() && !prev_is_barline && !prev_is_linebreak
                    && next.is_some() && next != Some(b'\n') && next != Some(b'|')
                {
                    self.events.push(Event::Gap(Gap { amount: (run_len - 1) as i32 }));
                }
                continue;
            }
            ...
            if ch == b'|' {
                ...
                if next == Some(b'|') {
                    if self.peek(self.pos + 2) == Some(b':') {
                        self.events.push(Event::Barline(Barline::new("repeat-start")));
                        self.pos += 3;
                    } else {
            ...
            if ch == b':' {
                let next = self.peek(self.pos + 1);
                if next == Some(b'|') {
                    if self.peek(self.pos + 2) == Some(b'|') && self.peek(self.pos + 3) == Some(b':') {
                        self.events.push(Event::Barline(Barline::new("repeat-both")));
                        self.pos += 4;
            ...
            if ch == b'<' {
                ...
                while self.pos < self.len() && self.input[self.pos] != b'>' {
                    ...
                    if is_whitespace_char(c) { self.pos += 1; continue; }
                    if c >= b'a' && c <= b'g' {
                        let cname = (c as char).to_string();
                        self.pos += 1;
                        let (accidental, octave, p2) = self.parse_note_pitch(self.pos);
                        self.pos = p2;
                        chord_notes.push(ChordNote { name: cname, accidental, octave });
                    } else {
                        self.pos += 1;
                    }
                }
                if self.pos < self.len() && self.input[self.pos] == b'>' { self.pos += 1; }
                ...
                if !chord_notes.is_empty() {
                    self.events.push(Event::Chord(Chord {
                        ...
                        duration, dots,
                        tie: att.tie,
                        slur_start: att.slur_start, slur_end: att.slur_end,
                        beam_start: att.beam_start, beam_end: att.beam_end,
                        articulations: att.articulations,
                        dynamic: att.dynamic,
                        hairpin: None, hairpin_start: false, hairpin_end: false,
                        trill: att.trill, trill_line: false, trill_start: false, trill_end: false,
                        grace: false, grace_slash: false,
                        ending: None, ending_start: false, ending_end: false,
                        fingering: att.fingering, fingering_position: att.fingering_position,
                        chord_symbol: att.chord_symbol,
                        staff_markers: att.staff_markers,
                        staff_text: att.staff_text,
                        expression_text: att.expression_text,
                        lyrics: att.lyrics,
                        tuplet_beats: 0.0, tuplet_number: 0, tuplet_count: 0,
                        tuplet_start: false, tuplet_end: false,
                        octave_line_number: 0, octave_line_direction: None,
                        octave_line_start: false, octave_line_end: false,
                    }));
                }
                continue;
            }
            ...
            if is_lower(ch) {
                ...
                while word_end < self.len() && is_word_char(self.input[word_end]) { word_end += 1; }
                ...
                if token == "end" && self.peek(word_end) == Some(b'{') {
                    ...
                    while label_pos < self.len() && self.input[label_pos] != b':' && self.input[label_pos] != b'}' {
            ...
            if ch == b'C' {
                let token = if self.peek(self.pos + 1) == Some(b'|') { "C|" } else { "C" };
                let end_pos = if token == "C|" { self.pos + 2 } else { self.pos + 1 };
            ...
            if ch == b'(' {
                if let Some(Event::Note(n)) = self.events.last_mut() { n.slur_start = true; }
                self.pos += 1;
                continue;
            }
            if ch == b')' {
                if let Some(Event::Note(n)) = self.events.last_mut() { n.slur_end = true; }
                self.pos += 1;
                continue;
            }
            ...
            if ch == b'{' {
                ...
                if !n_str.is_empty() {
                    ...
                    if self.peek(self.pos) == Some(b',') {
                        ...
                        if !m_str.is_empty() { tn = m_str.parse().unwrap_or(tn); }
                    }
                    if self.peek(self.pos) == Some(b':') { self.pos += 1; }
                    while self.pos < self.len() && is_whitespace_char(self.input[self.pos]) { self.pos += 1; }
            ...
            // Grace slash: /}
            if ch == b'/' && self.peek(self.pos + 1) == Some(b'}') && self.grace_open_order.is_some() {
            ...
            if ch == b'}' {
                ...
                if let Some(o) = self.tuplet_open_order { if o > latest_order { latest_order = o; close_kind = "tuplet"; } }
                if let Some(o) = self.octline_open_order { if o > latest_order { latest_order = o; close_kind = "octline"; } }
                if let Some(o) = self.ending_open_order { if o > latest_order { latest_order = o; close_kind = "ending"; } }
                if let Some(o) = self.hairpin_open_order { if o > latest_order { latest_order = o; close_kind = "hairpin"; } }
                if let Some(o) = self.trill_open_order { if o > latest_order { latest_order = o; close_kind = "trill"; } }
                if let Some(o) = self.grace_open_order { if o > latest_order { let _ = o; close_kind = "grace"; } }
            ...
            for i in start..end {
                match &mut self.events[i] {
                    Event::Note(n) => {
                        ...
                        if i == start { n.tuplet_start = true; }
                        if i == end - 1 { n.tuplet_end = true; }
                    }
                    Event::Rest(r) => {
                        r.tuplet_beats = self.tuplet_m as f64;
                        r.tuplet_number = self.tuplet_n;
                        r.tuplet_count = count;
                        if i == start { r.tuplet_start = true; }
                        if i == end - 1 { r.tuplet_end = true; }
                    }
                    Event::Chord(c) => {
                        c.tuplet_beats = self.tuplet_m as f64;
                        c.tuplet_number = self.tuplet_n;
                        c.tuplet_count = count;
                        if i == start { c.tuplet_start = true; }
                        if i == end - 1 { c.tuplet_end = true; }
                    }
                    _ => {}
                }
            }
    ...
    fn close_octline(&mut self) {
        if let Some(start) = self.octline_start_idx {
            let end = self.events.len();
            for i in start..end {
                let set_fields = |number: i32, dir: &Option<String>, is_start: bool, is_end: bool| {
                    (number, dir.clone(), is_start, is_end)
                };
                let (num, dir, is_start, is_end) = set_fields(
                    self.octline_number, &self.octline_dir, i == start, i == end - 1
                );
                match &mut self.events[i] {
                    Event::Note(n) => {
    ...
    fn close_ending(&mut self) {
        if let Some(start) = self.ending_start_idx {
            ...
            for i in start..self.events.len() {
                if !matches!(self.events[i], Event::LineBreak) { members.push(i); }
            }
            if !members.is_empty() {
                let first = members[0];
                ...
                for &i in &members {
                    ...
                    let l = label.clone();
                    match &mut self.events[i] {
                        Event::Note(n) => { n.ending = l; n.ending_start = is_first; n.ending_end = is_last; }
                        Event::Rest(r) => { r.ending = l; r.ending_start = is_first; r.ending_end = is_last; }
                        Event::Chord(c) => { c.ending = l; c.ending_start = is_first; c.ending_end = is_last; }
                        Event::Barline(b) => { b.ending = l; b.ending_start = is_first; b.ending_end = is_last; }
                        Event::Clef(cl) => { cl.ending = l; cl.ending_start = is_first; cl.ending_end = is_last; }
                        Event::TimeSig(t) => { t.ending = l; t.ending_start = is_first; t.ending_end = is_last; }
                        Event::Spacer(s) => { s.ending = l; s.ending_start = is_first; s.ending_end = is_last; }
                        _ => {}
                    }
                }
            }
    ...
    fn close_hairpin(&mut self) {
        if let Some(start) = self.hairpin_start_idx {
            ...
            for i in start..self.events.len() {
                if self.events[i].is_anchor() { anchors.push(i); }
            }
            if !anchors.is_empty() {
                let first = anchors[0];
                ...
                for &i in &anchors {
                    ...
                    let is_last = i == last;
                    match &mut self.events[i] {
                        Event::Note(n) => { n.hairpin = k; n.hairpin_start = is_first; n.hairpin_end = is_last; }
                        Event::Rest(r) => { r.hairpin = k; r.hairpin_start = is_first; r.hairpin_end = is_last; }
                        Event::Chord(c) => { c.hairpin = k; c.hairpin_start = is_first; c.hairpin_end = is_last; }
                        _ => {}
                    }
                }
            }
        ...
        if let Some(start) = self.trill_start_idx {
            ...
            if !anchors.is_empty() {
                ...
                for &i in &anchors {
                    ...
                    match &mut self.events[i] {
                        Event::Note(n) => { n.trill = true; n.trill_line = true; n.trill_start = is_first; n.trill_end = is_last; }
                        Event::Rest(r) => { r.trill = true; r.trill_line = true; r.trill_start = is_first; r.trill_end = is_last; }
                        Event::Chord(c) => { c.trill = true; c.trill_line = true; c.trill_start = is_first; c.trill_end = is_last; }
                        _ => {}
                    }
                }
            }
        ...
        if let Some(start) = self.grace_start_idx {
            ...
            for i in start..self.events.len() {
                if self.events[i].is_anchor() {
                    match &mut self.events[i] {
                        Event::Note(n) => { n.grace = true; n.grace_slash = slash; }
                        Event::Rest(r) => { r.grace = true; r.grace_slash = slash; }
                        Event::Chord(c) => { c.grace = true; c.grace_slash = slash; }
                        _ => {}
                    }
                }
            }
